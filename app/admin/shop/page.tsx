...
const categories = ["Apparel", "Accessories", "Equipment"]
...
export default function ShopAdmin() {
  ...
  function startAdd() {
    setEditing({ id: "", name: "", price: "", image: "/images/jersey.png", category: "Apparel" })
    setOpen(true)
  }
  ...
  async function save() {
    ...
    try {
      ...
      const payload = {
        ...editing,
        id: isNew ? `p-${Date.now()}` : editing.id,
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-heading text-3xl font-bold uppercase tracking-tight">Manage Shop</h1>
          <p className="text-sm text-muted-foreground">{rows.length} products listed</p>
        </div>
          ...
          {rows.map((p) => (
            <Card key={p.id} className="flex flex-col overflow-hidden p-0">
              <div className="relative h-36 overflow-hidden bg-secondary/30">
                <Image src={p.image || "/placeholder.svg"} alt="" fill className="object-contain p-4" />
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <p className="mt-1 font-heading font-bold text-primary">{p.price}</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Home Jersey 2025"
      ...
      >
        <div className="relative h-14 w-20 shrink-0 overflow-hidden rounded border border-border bg-muted">
          {uploading ? (
            ...
            <Image src={photo} alt="Preview" fill className="object-contain p-1" />
          ) : (
            <div className="flex h-full w-full items-center justify-center text-muted-foreground">
        ...
        <div className="flex flex-col gap-0.5 text-sm">
          <div className="flex items-center gap-1.5 font-medium text-foreground text-xs sm:text-sm">
            ...
            {uploading ? "Uploading..." : "Click or drag & drop product image"}
          </div>
          <div className="text-[11px] text-muted-foreground">PNG, JPG, or WEBP (up to 5MB)</div>
        </div>
      </div>
